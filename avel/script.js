(function () {
  ...
  function diagCol(title, items, listClass) {
    ...
    items.forEach(function (o) {
      ...
      var ico = el('span', 'ico', listClass === 'check-list' ? '✓' : (listClass === 'x-list' ? '!' : '•'));
  ...
  function renderDiagnostico(d) {
    ...
    diagCols.innerHTML = '';
    diagCols.appendChild(diagCol('Pontos fortes', d.diagnostico.pontosFortes, 'check-list'));
    diagCols.appendChild(diagCol('Pontos de atenção', d.diagnostico.pontosAtencao, 'x-list'));
    diagCols.appendChild(diagCol('Oportunidades', d.diagnostico.oportunidades, 'bullet-list'));

    var prioridadesEl = document.getElementById('blocoPrioridades');
    prioridadesEl.innerHTML = '';
    [['alta', 'Alta prioridade', 'danger', d.prioridades.alta], ['media', 'Média prioridade', 'warning', d.prioridades.media], ['baixa', 'Baixa prioridade', 'accent', d.prioridades.baixa]].forEach(function (row) {
      var col = el('div', 'prioridade-col ' + row[0]);
      col.appendChild(el('span', 'tag ' + row[2], row[1]));
      col.appendChild(ulFrom(row[3]));
